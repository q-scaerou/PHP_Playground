...
<html lang="en">
    ...
    <body>
        ...
        <?php
            $openPin = $_POST['admin_pin'];
            $cryptedPin= "";

            for ($i = 0; $i < strlen($openPin); $i++) {
                $pinPart = intval($openPin[$i]);
                $cryptedPin = $cryptedPin.strval(
                    ((($pinPart + $i + 1) % 10 + 10) % 10)
                );
            }

            $newAdmin=$mysqlClient->prepare('INSERT INTO administrateurs (admin_name, admin_pin) VALUES (?, ?)');
            $newAdmin->execute(array($_POST['admin_name'], $cryptedPin));    
        ?>

            <div class="mx-4">
                <h3 class="text-center">Le compte administrateur <?php echo $_POST['admin_name'] ?> a bien été créé <i class="bi bi-check2-circle"></i></h3>
            </div>
